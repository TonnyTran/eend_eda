...
data_root=/home3/theanhtran/corpus/Switchboard/Switchboard_SRE/LDC
musan_root=/home3/theanhtran/corpus/Switchboard/Switchboard_SRE/musan

# Modify simulated data storage area.
# This script distributes simulated data under these directories
simu_actual_dirs=(
/home3/theanhtran/corpus/Switchboard/test_simulated_data_Switchboard_SRE_8k
)
...
simu_opts_num_speaker_array=(1 2 3 4) #1 2 3 4 5 6 7 8
simu_opts_sil_scale_array=(2 2 5 9) # 2 2 5 9 14 20 27 35
simu_opts_rvb_prob=0.5
simu_opts_num_train=100 #100000
simu_opts_num_cv=20 #500
...
if [ $stage -le 0 ]; then
    ...
    if ! validate_data_dir.sh --no-text --no-feats data/swb_sre_comb; then
        ...
        # Prepare SWB.
        local/make_swbd2_phase1.pl $data_root/Switchboard-2P1-LDC98S75\
            data/swbd2_phase1_train
        local/make_swbd2_phase2.pl $data_root/Switchboard-2P2-LDC99S79 \
            data/swbd2_phase2_train
        local/make_swbd2_phase3.pl $data_root/Switchboard-2P3-LDC2002S06 \
            data/swbd2_phase3_train
        local/make_swbd_cellular1.pl $data_root/Switchboard-1-LDC2001S13 \
            data/swbd_cellular1_train
        local/make_swbd_cellular2.pl $data_root/Switchboard-Cell-P2-LDC2004S07 \
...
if [ $stage -le 2 ]; then
    ...
    for ((i=0; i<${#simu_opts_sil_scale_array[@]}; ++i)); do
        ...
        echo "`date "+%Y-%m-%d %H:%M:%S"` - Generate simulate mixture of $simu_opts_num_speaker speakers"
        for dset in swb_sre_tr swb_sre_cv; do #swb_sre_cv
            if [ "$dset" == "swb_sre_tr" ]; then
                n_mixtures=${simu_opts_num_train}
            else
                n_mixtures=${simu_opts_num_cv}
            fi
            ...
            if ! validate_data_dir.sh --no-text --no-feats $simudir/data/$simuid; then
                ...
                $simu_cmd --max-jobs-run 32 JOB=1:$nj $simudir/.work/make_mixture_$simuid.JOB.log \
                    $make_mixture_cmd --rate=$simu_rate \
        ...
        echo "`date "+%Y-%m-%d %H:%M:%S"` - Successfully simulate mixture of $simu_opts_num_speaker speakers"
    done
fi
