...
adapt_set=data/eval/dihard3_dev_1_4_spk_8k
adapt_valid_set=data/eval/dihard3_eval_1_4_spk_8k
...
if [ $stage -le 2 ]; then
    echo "averaging model parameters into $model_2spk_dir/$ave_2spk_id.nnet.npz"
    if [ -s $model_2spk_dir/$ave_2spk_id.nnet.npz ]; then
        echo "$model_2spk_dir/$ave_2spk_id.nnet.npz already exists. "
        echo " if you want to retry, please remove it."
        exit 1
    fi
    models=`eval echo $model_2spk_dir/snapshot_epoch-{$average_2spk_start..$average_2spk_end}`
    model_averaging.py $model_2spk_dir/$ave_2spk_id.nnet.npz $models || exit 1
fi
...
if [ $stage -le 3 ]; then
    echo "Stage3: training model at $model_dir" 
    if [ -d $model_dir ]; then
    ...
    $train_cmd $work/train.log \
        train.py \
            -c $train_config \
            --initmodel $model_dir/avg18.nnet.npz \
            $train_args \
...
if [ $stage -le 4 ]; then
    ...
    models=`eval echo $model_dir/snapshot_epoch-{$average_start..$average_end}` #{$average_start..$average_end}
...
echo "Inference on DIHARD3 dataset!"
if [ $stage -le 7 ]; then
