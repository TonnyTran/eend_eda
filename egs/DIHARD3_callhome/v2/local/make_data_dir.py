...
def read_rttm_file(rttm_path):
    ...
    with open(rttm_path, 'r') as f:
        ...
        for n, line in enumerate(f, start=1):
            ...
            speaker_id = f'{recording_id}_{fields[7]}'
            utterance_id = f'{speaker_id}_{int(onset*100):07d}_{int(offset*100):07d}'
            duration = float(fields[4])
            turns.append(
                RTTMTurn(recording_id, speaker_id, onset, offset, line, duration, utterance_id))
    return turns
...
def write_rttm_file(rttm_path, turns):
    ...
    with open(rttm_path, 'w') as f:
        ...
        turns = sorted(turns, key=lambda x: x.utterance_id)
        for turn in turns:
            line = (f'SPEAKER   {turn.recording_id}     1   {turn.onset}    {turn.duration}  <NA>    <NA>    {turn.speaker_id}   <NA>\n')
            f.write(line)
...
def write_utt2spk(utt2spk_path, recordings):
    ...
    with open(utt2spk_path, 'w') as f:
        ...
        for recording in recordings:
            turns = sorted(recording.turns, key=lambda x: x.utterance_id)
            for turn in turns:
                line = f'{turn.utterance_id} {turn.speaker_id}\n'
                f.write(line)
...
def write_segments_file(segments_path, recordings):
    ...
    with open(segments_path, 'w') as f:
        ...
        for recording in recordings:
            turns = sorted(recording.turns, key=lambda x: x.utterance_id)
            for turn in turns:
                line = (f'{turn.utterance_id} {recording.recording_id} '
                        f'{turn.onset} {turn.offset}\n')
                f.write(line)
