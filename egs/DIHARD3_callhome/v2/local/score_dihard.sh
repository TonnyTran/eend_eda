...
echo "usage: $0 score CORE set - Collar size = 0.00 s"
...
for domain in audiobooks broadcast_interview clinical court cts maptask meeting restaurant socio_field socio_lab webvideo; do
    local/rttm_from_uem.py $refer_rttm $release_dir/data/uem_scoring/core/$domain.uem $tmpdir ref
    local/rttm_from_uem.py $sys_rttm $release_dir/data/uem_scoring/core/$domain.uem $tmpdir sys
...
for domain in one_spk two_spk three_spk four_spk five_to_ten; do
    local/rttm_from_uem.py $refer_rttm $release_dir/data/uem_scoring/core/num_spk/$domain.uem $tmpdir ref
    local/rttm_from_uem.py $sys_rttm $release_dir/data/uem_scoring/core/num_spk/$domain.uem $tmpdir sys
...
echo "usage: $0 score CORE set - Collar size = 0.25 s"
echo "$0: ***** RESULTS *****   DER          MISS       FA          ERR"
